...
| **I-1.2** | Flyway migration `V5__create_password_reset_tokens.sql` (table `iam_password_reset_tokens` với FK `user_id`, UNIQUE `token_hash`, `expires_at`, `used`, `used_at`) | [ ] | E2E gọi `/auth/forgot-password` → token lưu DB với hash SHA-256; `/auth/reset-password` → token mark `used=true` |
| **I-1.3** | `application.yml` (cả dev/prod profile): đổi `spring.jpa.hibernate.ddl-auto=update` → `validate`. Hibernate sẽ KHÔNG tự ý ALTER schema; mismatch schema ↔ entity → app fail ngay ở startup với lỗi rõ ràng | [ ] | App vẫn start khi schema khớp entity; thiếu cột/thừa cột/sai kiểu → fail rõ "Schema-validation: missing column X" |
...
| **I-1.2** | Tạo `V5__create_password_reset_tokens.sql`: bảng `iam_password_reset_tokens (id UUID PK, user_id UUID NOT NULL FK→iam_users.id, token_hash VARCHAR(64) NOT NULL UNIQUE, expires_at TIMESTAMPTZ(6) NOT NULL, used BOOLEAN NOT NULL DEFAULT FALSE, used_at TIMESTAMPTZ(6) NULL, created_at, updated_at, version, deleted, deleted_at, created_by, updated_by)` với index `(user_id)`, index `(expires_at)`. **Quan trọng**: `token_hash` lưu SHA-256 hex (64 char) của token raw — không lưu raw token. Update `IamFacadeImpl.resetPassword(...)` để dùng bảng mới (hiện đang dùng `PasswordResetTokenJpaEntity` đã map sẵn — chỉ cần đảm bảo DB match) | E2E gọi `/auth/forgot-password` → token lưu DB với hash SHA-256; `/auth/reset-password` → token mark `used=true` |
| **I-1.3** | Sửa `application.yml` (cả profile `default`, `dev`, `prod`): đổi `spring.jpa.hibernate.ddl-auto=update` → `validate`. Đảm bảo `@EnableJpaAuditing` không can thiệp (chỉ touch `created_at`/`updated_at` ở runtime). Nếu mismatch, app sẽ fail ở startup với exception rõ ràng kiểu `SchemaManagementException: Schema-validation: missing column [xxx]` — KHÔNG âm thầm sửa. **Lưu ý**: trước khi chuyển, **bắt buộc chạy migration I-1.1 + I-1.2 đầy đủ** trên mọi môi trường, nếu không app sẽ fail ngay. Có thể cần `spring.jpa.properties.hibernate.hbm2ddl.schema_generation.create_schemas=false` để chắc chắn | App vẫn start khi schema khớp entity; thiếu cột/thừa cột/sai kiểu → fail rõ ràng ngay startup |
