...
| **I0.1** | Refresh token rotation: mỗi refresh sinh token mới, invalidate token cũ. Lưu JTI vào Redis (`refresh:token:{jti}` → userId, TTL = refresh TTL). **Không có migration DB** (dùng Redis, không tạo entity). | [x] | Refresh x2 lần liên tiếp với cùng token cũ → trả `AUTH_TOKEN_INVALID` (401); reset/change password → mọi refresh token cũ bị revoke *(E2E pending — sếp chốt để test sau)* |
| **I0.2** | Brute-force lockout: 5 lần login sai → khóa 15 phút theo **email + IP** song song (Redis key `login:fail:email:{email}` + `login:fail:ip:{ip}`, lock key `login:lock:email:{email}` / `login:lock:ip:{ip}`) | [x] | Login sai 5 lần từ 1 IP/email → lần 6 trả 429 `AUTH_ACCOUNT_LOCKED` (email) hoặc `AUTH_IP_LOCKED` (IP); login thành công → reset cả 2 lock *(E2E pending)* |
| **I0.3** | Rate limit **4 endpoint** (`/register`, `/forgot-password`, `/resend-otp`, `/reset-password`): 3 req/IP/giờ (Redis key `ratelimit:{endpoint}:{ip}`, TTL 1h) — triển khai qua `@RateLimited(endpoint = "...")` + `RateLimitAspect` | [x] | Spam 4 lần trong 1h từ 1 IP → lần 4 trả 429 `AUTH_RATE_LIMIT_EXCEEDED` với placeholder `{0}` retry-after-seconds; IP khác vẫn OK *(E2E pending)* |
| **I0.4** | Audit endpoint matrix: tách `public-paths` trong `application.yml` (bỏ wildcard `/api/v1/auth/**`, liệt kê 8 path public + actuator/swagger); viết `docs/auth-endpoint-matrix.md` (11 endpoint × matrix). `SecurityConfig.anyRequest().authenticated()` đã có sẵn → không cần sửa code | [x] | Request tới `/auth/logout` không có token → `AuthEntryPoint` trả 401 (không qua controller); request tới `/auth/login` không có token → 200 OK *(E2E pending)* |
| **I1.1** | `outbox/api/`: `OutboxWriter` interface (`enqueue(topic, key, payload)`) + `OutboxEventPayload` record + `@TransactionalEventListener` config (I1.1 chỉ tạo module + interface; JPA entity + migration V6 ở I1.2; IAM wire ở I1.7) | [x] | Module `outbox` compile độc lập; `mvn -pl bootstrap -am clean install` exit 0 *(E2E pending — I1.7 sẽ verify end-to-end khi wire IAM)* |
| **I1.2** | `outbox/infrastructure/persistence/`: `OutboxEventJpaEntity` + repo + Flyway migration `V6__create_outbox_events.sql` (id, aggregate_type, aggregate_id, topic, payload JSONB, status, retry_count, created_at, sent_at, last_error) | [x] | `mvn -pl bootstrap -am clean install` exit 0; Flyway tree có V2 → V6 *(E2E pending — sẽ verify schema khi apply migration)* |
| **I1.3** | `outbox/infrastructure/scheduler/`: `OutboxRelayScheduler` (poll 5s, claim batch `SELECT FOR UPDATE SKIP LOCKED`, publish Kafka, mark SENT) + `OutboxKafkaConfig` (topic `notification.email.v1`) | [x] | `mvn -pl bootstrap -am clean install` exit 0; scheduler + Kafka config load OK *(E2E pending — sẽ verify publish khi có Kafka broker)* |
...
| **I1.6** | `notification/infrastructure/messaging/`: `KafkaEmailConsumer` listen `notification.email.v1` → resolve template theo `payload.templateName` + `payload.locale` → render Thymeleaf → gọi `MailService` | [x] | Outbox → Kafka → mail đến inbox MailHog |
...
| **I1.8** | Tạo `OutboxJpaWriter` (persist event vào `outbox_events` table, profile `!test`). Sửa `application.yml`: Gmail SMTP config (`smtp.gmail.com:587`, TLS, credentials từ `.env`). `LoggingOutboxWriter` giữ lại cho profile `test`. `.env` đã có `SPRING_MAIL_HOST=smtp.gmail.com`, `SPRING_MAIL_PORT=587`, credentials App Password. | `docker compose up -d postgres redis kafka` → register user → OTP mail đến inbox Gmail thật (5-15s). Forgot password → reset link mail đến. |
| **I1.9** | Tạo `OutboxRetryConfig` (`@ConfigurationProperties(prefix = "app.outbox.retry")`) với `maxAttempts=3`, `backoffSeconds=[1,5,30]`. Sửa `OutboxRelayScheduler.publishToKafka()`: thay exponential backoff bằng fixed backoff từ config, mark `FAILED` khi `retryCount >= maxAttempts`. Thêm `app.outbox.retry.*` config vào `application.yml`. | Test: stop Kafka → enqueue event → 1s sau event PENDING (retry 1) → restart Kafka → event SENT. Stop → 5s sau retry 2 → 30s sau retry 3 → mark FAILED. |
