  ...
  public TriggerAnonymizationResponse triggerAnonymization() {
    ...
    for (User user : usersToAnonymize) {
      try {
        ...
        count++;
        log.info("USER_ANONYMIZED_SUCCESS: userId={}", com.pwb.backend.iam.internal.application.helper.PiiScrubber.userRef(user.getId()));
      } catch (Exception e) {
        log.error("USER_ANONYMIZATION_FAILED: userId={}", com.pwb.backend.iam.internal.application.helper.PiiScrubber.userRef(user.getId()), e);
      }
    }
